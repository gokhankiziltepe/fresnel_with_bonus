 ...
 * q --> turn batman left		*
 * e --> turn batman right		*
 * g --> decrease reflectance	*
 * h --> increase reflectance	*
 * t --> decrease power			*
 * y --> increase power			*
 *------------------------------*
 * 
 ...
 * the executable output 'fresnel_with_bonus' will be created 
 * 
 * You can also use this line from terminal
 * #g++ -Wall main.cpp -lGLEW -lGL -lglut -lGLU -lSOIL -lassimp -lm -o fresnel_with_bonus 
 * 
 * Changed shaders are
...
static void timerCallback (int value)
{
    if (0 != value) {
		...
		sprintf(
                TempString,
                "%s: %d Frames Per Second @ %d x %d, Reflectance: %.2f, Power: %.2f",
                WINDOW_TITLE_PREFIX,
                FrameCount * 4,
                CurrentWidth,
                CurrentHeight,
