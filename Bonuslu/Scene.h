...
public:
    ...
    void draw(float reflectance, float power){
        ...
        for(  unsigned long i = 0; i < len ; i++)
        {
            drawNode(children[i], reflectance, power);
        }
        if(effects.size()>0)
        {
...
private:
    ...
    Camera *camera;
    void drawNode(SceneNode *node, float reflectance, float power){
        node->draw(camera, reflectance, power); 
        
        unsigned long len = node->children.size();
        for(  unsigned long i = 0; i < len ; i++)
        {
            drawNode(node->children[i], reflectance, power);
        }    
    }
};
